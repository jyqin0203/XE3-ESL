...
const outputDir = path.join(rootDir, 'assets/speakup/online');
const framesDir = path.join(outputDir, 'frames');
const outputGlbPath = path.join(outputDir, 'speakup-online-intro.glb');
const outputAtlasPath = path.join(outputDir, 'speakup-online-atlas.png');
const outputContactSheetPath = path.join(
  outputDir,
  'speakup-online-ui-flow.png',
);
...
function verifyExpectedOnlineScene(document) {
  ...
  if (
    ...
    throw new Error('The Online scene no longer matches the reviewed source structure.');
  }
}
...
async function decodeKtx2ToPng(ktx2, sharp, BASIS) {
  ...
  try {
    if (!file.isValid() || !file.isETC1S()) {
      throw new Error('Expected the reviewed ETC1S Online atlas.');
    }
    if (
      file.getWidth() !== 1024 ||
      file.getHeight() !== 1024 ||
      ...
      throw new Error('Unexpected Online atlas dimensions or alpha mode.');
    }
...
function replaceAtlas(chunks, binChunk, document, atlasPng) {
  ...
  if (
    ...
    throw new Error('The source Online atlas bufferView is missing or shared.');
  }
  ...
  document.images[0] = {
    name: 'SpeakUp Online Practice Flow Atlas',
...
async function main() {
  ...
  verifyExpectedOnlineScene(document);
  ...
  if (invariantSnapshot(document) !== originalInvariant) {
    throw new Error(
      'Online nodes, meshes, materials, skins, animations, scenes, or accessors changed.',
    );
  }
  ...
  if (
    ...
    throw new Error('Encoded output did not preserve the reviewed Online scene.');
  }
