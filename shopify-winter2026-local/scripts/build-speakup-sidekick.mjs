...
const outputDir = path.join(rootDir, 'assets/speakup/sidekick');
...
const MODEL_SPECS = [
  {
    ...
    output: path.join(outputDir, 'speakup-sidekick-high.glb'),
    atlas: path.join(outputDir, 'sidekick-atlas-high.png'),
  },
  {
    id: 'medium',
    source: path.join(
    ...
    output: path.join(outputDir, 'speakup-sidekick-medium.glb'),
    atlas: path.join(outputDir, 'sidekick-atlas-medium.png'),
  },
];
...
const FALLBACK_SPECS = [
  {
    ...
    output: path.join(outputDir, 'sidekick-desktop.jpg'),
    optimized: path.join(outputDir, 'sidekick-desktop.ktx2'),
    width: 2880,
    height: 1780,
  ...
  },
  {
    ...
    output: path.join(outputDir, 'sidekick-mobile.jpg'),
    optimized: path.join(outputDir, 'sidekick-mobile.ktx2'),
    width: 804,
    height: 1748,
...
function verifyExpectedScene(document) {
  ...
  if (
    ...
    throw new Error('The Sidekick scene no longer matches the reviewed structure.');
  }
}
...
function disableBlinkOverlay(document) {
  ...
  if (!eyelids || eyelids.mesh === undefined) {
    throw new Error('Expected the visible sidekick-eyelids mesh.');
  }
  delete eyelids.mesh;
}
...
async function decodeKtx2(ktx2, sharp, BASIS) {
  ...
  try {
    if (!file.isValid() || !file.isETC1S()) {
      throw new Error('Expected the reviewed ETC1S Sidekick atlas.');
    }
    const width = file.getWidth();
    const height = file.getHeight();
    if (width !== 2048 || height !== 2048 || !file.getHasAlpha()) {
      throw new Error('Unexpected Sidekick atlas dimensions or alpha mode.');
    }
...
function replaceEmbeddedKtx2(parsed, replacement) {
  ...
  if (oldSpanEnd < end || oldSpanEnd - end >= suffixAlignment) {
    throw new Error('Unexpected padding after the Sidekick texture bufferView.');
  }
  const oldGap = binChunk.data.subarray(end, oldSpanEnd);
  if ([...oldGap].some((byte) => byte !== 0)) {
    throw new Error('Non-zero bytes found in Sidekick texture alignment gap.');
  }
...
async function buildAtlasModel(spec, sharp, BASIS, basisEncoder) {
  ...
  const encodedKtx2 = path.join(outputDir, `.sidekick-${spec.id}.ktx2`);
  ...
  const manifest = {
    ...
    intent: {
      changed: [
        ...
        'Replace the Shopify laptop mark with the existing white SpeakUp mark.',
        'Detach the sidekick-eyelids mesh so the character remains naturally open-eyed.',
      ],
      preserved: [
